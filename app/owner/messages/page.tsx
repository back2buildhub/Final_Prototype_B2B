...
export default function OwnerMessages() {
  ...
  useEffect(() => {
    async function init() {
      ...
      localStorage.setItem('b2b_owner_messages_seen', new Date().toISOString())
      
      // Auto-select conversation from URL param
      const params = new URLSearchParams(window.location.search)
      const convId = params.get('conv')
      if (convId) {
        // Find in loaded conversations
        const found = convs.find((c: any) => c.id === convId)
        if (found) {
          openConversation(found)
        } else {
          // Conversation exists in DB but not in list yet — load it directly
  ...
  // Load all conversations for this owner — with separate profile queries
  async function loadConversations(userId: string) {
    ...
    // Enrich each conversation with project and constructor profile
...
function renderBody(body: string) {
  ...
  // Load project info and constructor profile for a conversation
  async function enrichConversation(conv: any) {
    const [{ data: project }, { data: profile }] = await Promise.all([
      ...
      supabase.from('profiles').select('full_name, city').eq('id', conv.constructor_id).single(),
    ])
    return { ...conv, project, constructor_profile: profile }
  }
  ...
  async function loadMessages(convId: string) {
    ...
    const channel = supabase.channel(`owner-${convId}`)
  ...
  const constructorName = (conv: any) => conv?.constructor_profile?.full_name || 'Constructor'
          ...
          {!loading && conversations.length === 0 && (
            <div style={{ padding: '24px 16px', textAlign: 'center' }}>
              ...
              <p style={{ color: '#64748B', fontSize: '13px', lineHeight: '1.6' }}>
                No conversations yet. Go to a project and click <strong style={{ color: '#E2E8F0' }}>💬 Message Constructor</strong> to start chatting.
              </p>
            </div>
          )}
      ...
      {!selected ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#64748B', gap: '10px' }}>
          ...
          <p style={{ fontSize: '12px', color: '#475569', textAlign: 'center', maxWidth: '260px', lineHeight: '1.6' }}>
            Or click <strong style={{ color: '#E2E8F0' }}>💬 Message Constructor</strong> on any project to open a conversation here
          </p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #1E3A5A', display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
            <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: '#2D1A0E', border: '1px solid #7C2D12', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>
              🏗️
            </div>
            <div>
              <div style={{ fontWeight: '600', fontSize: '15px' }}>{constructorName(selected)}</div>
            ...
            <button
              onClick={() => {
                ...
                setSelected(null)
                setMessages([])
                window.history.pushState({}, '', '/owner/messages')
              }}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {messages.length === 0 && (
              <div style={{ textAlign: 'center', paddingTop: '50px', color: '#64748B' }}>
                ...
                <p style={{ fontSize: '13px', color: '#475569' }}>Say hello to {constructorName(selected)}.</p>
              </div>
            )}
            {messages.filter(m => !deletedMsgIds.has(m.id)).map((m, i) => (
              <div key={m.id}
                ...
                <div style={{
                  ...
                  background: m.sender_id === user?.id ? '#1A3050' : '#111E2E',
                  border: m.sender_id === user?.id ? 'none' : '1px solid #1E3A5A',
                }}>
                  {renderBody(m.body)}
          ...
          <form onSubmit={sendMessage} style={{ padding: '14px 20px', borderTop: '1px solid #1E3A5A', display: 'flex', gap: '8px', alignItems: 'flex-end', flexShrink: 0 }}>
            ...
            <input
              ...
              placeholder={`Message ${constructorName(selected)}...`}
              style={{ flex: 1 }}
            />
