...
const QUICK_PROMPTS = [
  '🧱 How many tiles do I need for a 252 sq ft room using 2ft × 2ft tiles?',
  '💰 What is the estimated cost to renovate a kitchen in Colombo?',
  '📅 How long does a full bathroom renovation typically take?',
  '📋 What permits do I need for a house extension in Sri Lanka?',
  '🏠 Is it better to renovate or rebuild an old house?',
  '🔒 How does the escrow payment system protect me as a property owner?',
]
...
export default function OwnerAI() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // If coming from a project, load project context
      if (projectId) {
  ...
  async function openConversation(conv: Conversation) {
    setActiveConvId(conv.id)
    setMessages([])
    // Clear project context when manually switching conversations
    setProjectContext(null)
    setProjectBanner(null)
    window.history.pushState({}, '', '/owner/ai')
    const { data } = await supabase
      .from('ai_messages')
      .select('*')
      .eq('conversation_id', conv.id)
      .order('created_at', { ascending: true })
    setMessages((data || []).map(m => ({ role: m.role, content: m.content, intentLabel: m.intent_label })))
  }
  ...
  if (!isPro) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '500px' }}>
      <div style={{ textAlign: 'center', maxWidth: '400px' }}>
        ...
        <a href="/owner/profile" style={{ background: '#E8622A', color: '#fff', padding: '12px 28px', borderRadius: '8px', textDecoration: 'none', fontWeight: '700', fontSize: '15px' }}>
          ⚡ Upgrade to Pro
        </a>
      </div>
    </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', minHeight: '0' }}>
          {conversations.length === 0 && (
            <p style={{ padding: '16px 14px', color: '#475569', fontSize: '12px', lineHeight: '1.6' }}>
              No conversations yet. Click a project's 🤖 Ask Bass AI button or start a new chat.
            </p>
          )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '14px 20px', borderBottom: '1px solid #1E3A5A', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#64748B', marginTop: '1px' }}>Construction assistant for property owners</div>
          </div>
          <span style={{ background: '#431407', color: '#FB923C', border: '1px solid #7C2D12', padding: '3px 10px', borderRadius: '4px', fontSize: '11px', fontWeight: '600' }}>
            Property Owner
          </span>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px', minHeight: '0' }}>
          ...
          {!activeConvId && (
            <div style={{ textAlign: 'center', paddingTop: '30px' }}>
              ...
              <h2 style={{ fontSize: '20px', fontWeight: '700', marginBottom: '8px' }}>What are you building today?</h2>
              <p style={{ color: '#64748B', fontSize: '14px', maxWidth: '400px', margin: '0 auto 28px', lineHeight: '1.6' }}>
                Ask Bass AI for cost estimates, material calculations, project advice, and construction guidance for Sri Lanka.
                Or click <strong style={{ color: '#FB923C' }}>🤖 Ask Bass AI</strong> on any project for context-aware answers.
              </p>
          ...
          {messages.map((m, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', gap: '10px', alignItems: 'flex-start' }}>
              ...
              <div style={{ padding: '12px 16px', maxWidth: '75%', fontSize: '14px', lineHeight: '1.6', borderRadius: m.role === 'user' ? '16px 16px 2px 16px' : '16px 16px 16px 2px', background: m.role === 'user' ? '#1A3050' : '#111E2E', border: m.role === 'user' ? 'none' : '1px solid #1E3A5A' }}>
                {m.role === 'assistant' && (
                  <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: '10px', background: '#082550', color: '#60A5FA', border: '1px solid #1E3A5A', padding: '1px 7px', borderRadius: '10px', fontWeight: '600' }}>
                      Property Owner
                    </span>
                  </div>
                )}
              ...
              {m.role === 'user' && (
                <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#0B2924', border: '1px solid #0F6B52', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', flexShrink: 0, marginTop: '2px' }}>🏠</div>
              )}
            </div>
          ))}
        ...
        <div style={{ padding: '14px 20px', borderTop: '1px solid #1E3A5A', flexShrink: 0 }}>
          {projectBanner && (
            <div style={{ fontSize: '11px', color: '#7C2D12', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span>📁</span>
          ...
          <form onSubmit={e => { e.preventDefault(); sendMessage() }} style={{ display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
            <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
              ...
              placeholder={projectBanner ? `Ask Bass AI about ${projectBanner.title}...` : 'Ask about materials, costs, timelines, permits... (Enter to send)'}
